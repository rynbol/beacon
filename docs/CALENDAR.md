...
Stability correction after repeated rendering reports: date/week content animations are removed, and date replacement explicitly disables inherited animations while closing the previous card. Full-notes expansion now uses a direct five-line/full-text layout change, without animated measured clipping. Measurements only determine disclosure visibility. This deliberately removes the problematic date/week and full-notes motion. Main/preview builds passed; sample UI checked full expansion/collapse and week changes with an open event. Snapshot checks establish correct end states, not frame-by-frame animation quality.
